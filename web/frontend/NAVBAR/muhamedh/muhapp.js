...
const API_URL =
    `/api/map?office_id=1&limit=9999&_muhamedkhanova=1`;

const USER_INFO_URL_PREFIX =
    "https://muhamedkhanova.app.enes.tech/api/v2/map/";

const SEED_CAP_TOKEN =
    "";

const TOKEN_STORAGE_KEY = "muhamedkhanova.map.capToken.v2";
const LEGACY_TOKEN_STORAGE_KEYS = [
    "muhamedkhanova.map.token"
];
...
const VIEWBOX_WIDTH = 2225;
const VIEWBOX_HEIGHT = 3050;
const GRID_UNIT = 70;
const VB_MIN_X = -560;
const VB_MIN_Y = -1725;
const VB_W = 2225;
...
// Edit positions in: /web/frontend/NAVBAR/muhamedh/positions.json
...
async function getAuthToken({ forceRefresh = false } = {}) {
    ...
    try {
        const json = await fetchJson("/api/token?_muhamedkhanova=1");
...
function renderPCs(pcs) {
    ...
    pcs.forEach(pc => {
        ...
        const leftPercent = ((point.x - VB_MIN_X) / VB_W) * 100;
        const topPercent = ((point.y + PC_Y_OFFSET - VB_MIN_Y) / VB_H) * 100;
...
function getZoneForPC(num) {
    ...
    if (n >= 1 && n <= 35) return '1-35';
    if (n >= 36 && n <= 65) return '36-65';
    if (n >= 66 && n <= 70) return '66-70';
    if (n >= 71 && n <= 75) return '71-75';
    if (n >= 79 && n <= 88) return '79-88';
    if (n >= 89 && n <= 92) return '89-92';
    if (n >= 99 && n <= 101) return '99-101';
    if (n >= 105 && n <= 109) return '105-109';
    return null;
}
...
async function fetchPCUserInfo(pc, _token) {
    ...
    try {
        return await fetchJson(`${BACKEND_URL}/api/user-info?pc_id=${pc.id}&_muhamedkhanova=1`);
    } catch (err) {
