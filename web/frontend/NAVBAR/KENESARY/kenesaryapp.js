...
const USER_INFO_URL_PREFIX =
    "https://primekenesary.app.enes.tech/api/v2/map/";

const SEED_CAP_TOKEN =
    "";

const TOKEN_STORAGE_KEY = "primekenesary.map.capToken.v2";
const LEGACY_TOKEN_STORAGE_KEYS = [
    "primekenesary.map.token"
];
...
async function getAuthToken({ forceRefresh = false } = {}) {
    ...
    try {
        const json = await fetchJson("/api/token?_primekenesary=1");
...
function getZoneForPC(num) {
    ...
    if (n >= 1 && n <= 80) return '1-80';
    if (n >= 81 && n <= 94) return '81-94';
    if (n >= 95 && n <= 106) return '95-106';
    return null;
}
...
async function fetchPCUserInfo(pc, _token) {
    ...
    try {
        return await fetchJson(`${BACKEND_URL}/api/user-info?pc_id=${pc.id}&_primekenesary=1`);
    } catch (err) {
